...
```{r horse-plot,out.width="90%", fig.cap="Horse populations for all provinces in Canada from 1906 - 1972"}
...
knitr::kable(horses_sd_table,caption = "horse table")
```
...
Additionally, note that in Table \@ref(tab:horse-table) we consider the sample standard deviation of the number of horses during the same time span as Figure \@ref(fig:horse-plot).

```{r horse-pop-large-sd-plot,out.width="90%", fig.cap="Horse populations for the province with the largest standard deviation"}
knitr::include_graphics("../../results/horse_pop_plot_largest_sd.png")
```


In Figure \@ref(fig:horse-pop-large-sd-plot) we zoom in and look at the province of `r largest_sd`, which had the largest spread of values in terms of standard deviation.
